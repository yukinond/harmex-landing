...
<template>
  <div :class="{ 'px-1.5': route.name === 'index' }">
    <header
      id="header"
      class="flex justify-between items-center h-[84px] px-3 bg-white"
      :class="{
        'mb-1.5 rounded-b-xl shadow-[2px_1px_7px_1px_rgba(0,_0,_0,_0.1)]':
          route.name === 'index',
        'lg:px-[calc(10%)]': route.name !== 'index',
      }"
    >
      <div class="flex gap-6">
        ...
        <div class="dropdown lg:flex hidden">
          <button
            ...
            class="dropbtn font-[500] text-[14px] leading-[17.5px] flex my-auto gap-1.5"
          ...
          <Transition>
            <div
              v-if="popupVisible"
              class="dropdown-content full-width mt-8"
              id="myDropdown"
            >
              <div
                class="w-full rounded-md p-6 flex bg-white shadow-[0px_-1px_5px_-2px_rgba(0,_0,_0,_0.1)]"
              >
                <div class="flex flex-col gap-4 font-normal w-[25%] mr-6">
                  <button
                    ...
                    :key="item.value"
                    class="flex justify-between items-center w-full hover:text-[--primary] group"
                    :class="{ 'text-[--primary]': item.value === currentPopup }"
                    @click="currentPopup = item.value"
                  >
                    <div class="flex gap-1.5 items-center">
                      <Icon
                        :name="item.icon"
                        class="w-5 h-5 text-[#7c7c7c] group-hover:text-[--primary]"
                        :class="{
                          'text-[--primary]': item.value === currentPopup,
                        }"
                      />
                ...
                <div class="flex flex-col gap-4 ml-6">
                  <button
                    type="button"
                    :aria-label="`Перейти ${subItem.title}`"
                    @click="navigateToSite(subItem.value, subItem.disabled)"
                    :disabled="subItem.disabled"
                    v-for="subItem in popupData.find(
                      (item) => item.value === currentPopup
                    ).subItems || []"
                    :key="subItem.value"
                    class="flex gap-1.5 items-center disabled:text-[#f4f4f4]"
                  >
                    <span
                      class="font-[400] text-[14px] leading-[26px] text-[#303940]"
                      >{{ subItem.title }}</span
                    >
                  </button>
                </div>
        ...
        <nav
          class="items-center gap-6 font-[500] text-[14px] leading-[17.5px] lg:flex hidden"
        >
          <Nuxt-Link to="/blog">Блог</Nuxt-Link>
          <Nuxt-Link to="/#reviews">Кейсы</Nuxt-Link>
          <Nuxt-Link :to="route.name === 'index' ? '#contact' : '/#footer'"
            >Помощь</Nuxt-Link
          >
      ...
      <div class="h-full lg:flex hidden">
        <div
          ...
          :class="{
            'px-1.5 border-l border-l-[#E8E8E8]': route.name === 'index',
          }"
        >
          <a
        ...
        <div
          class="flex items-center h-full"
          :class="{
            'pl-3 border-l border-l-[#E8E8E8] ': route.name === 'index',
          }"
        >
          <a
            type="button"
      ...
      <Transition name="slide-down">
        ...
        :href=targetUrlAuth
          ...
          >
            ...
            <Nuxt-Link class="text-left" to="/#reviews" @click="toggleOpen"
              >Кейсы</Nuxt-Link
            >
          ...
          <div class="dropdown">
            ...
            <Transition>
              <div
                v-if="popupVisible"
                class="dropdown-content full-width mt-2"
                id="myDropdownMobile"
              >
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          :disabled="subItem.disabled"
                          @click="
                            navigateToSite(subItem.value, subItem.disabled)
                          "
    ...
    <footer
      id="footer"
      class="mt-1.5 rounded-xl rounded-b-none bg-[#323232] px-6 py-8 lg:p-16 flex flex-col gap-12"
    >
      <img
        alt="log-white"
      ...
      <div class="flex sm:flex-row flex-col justify-between gap-12">
        <div class="flex sm:flex-row flex-col gap-12 lg:gap-20 flex-wrap">
          <div
            v-for="item in popupData.slice(0, 4)"
            :key="item.value"
            class="flex flex-col gap-3 font-[600] text-[14px] leading-[18px]"
          >
            <p class="text-white">{{ item.title }}</p>
            <div
              v-for="subItem in item.subItems.slice(0, 7)"
              :key="subItem.value"
              class="flex flex-col gap-3 text-[#ADADAD]"
            >
              <button
                type="button"
                @click="navigateToSite(subItem.value, subItem.disabled)"
                class="font-[400] text-[14px] leading-[17.5px] lg:leading-[26px] text-[#E8E8E8] text-left"
                :class="{
                  'cursor-not-allowed': subItem.disabled,
                  'hover:text-[--primary]': !subItem.disabled,
                }"
              >
                {{ subItem.title }}
              </button>
        ...
        <div class="flex flex-col gap-4 items-end text-white">
          <a
            class="text-[32px] leading-[38px] lg:text-[40px] lg:leading-[48px] font-[600]"
            href="tel:8 (964) 726 50 61"
            >+7 964 726-50-61</a
          >
          <a
            class="font-[500] text-[14px] leading-[17.5px] flex gap-3 items-center"
          ...
          >
          <a
            class="mb-2 text-[16px] text-[#47A4D2]"
          ...
          <div class="flex gap-3 items-center">
            <span class="text-[14px] text-[#ADADAD]"> Наверх </span>
            <button
              type="button"
              aria-label="Наверх"
              @click="scrollTop"
              class="btn-primary max-h-[42px] flex items-center"
            >
              <Icon name="octicon:arrow-up-24" class="w-5 h-7 text-black" />
            </button>
          </div>
        </div>
      </div>
      <div
        class="border-t border-[#4B4B4B] pt-12 flex lg:flex-row flex-col justify-center lg:justify-between text-[14px] leading-[17.5px] gap-12"
      >
        <p class="text-white text-center lg:text-left">
          ©{{ new Date().getFullYear() }} HARMEX Все права защищены.
        </p>
        <div
          class="flex lg:flex-row flex-col gap-6 items-center lg:items-end text-[#ADADAD]"
        >
          <a
