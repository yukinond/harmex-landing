...
const { formattedContent, getHeadingClass, getClass } =
  useFormattedContent(article);
...
<template>
  <div class="flex lg:flex-row flex-col justify-between">
      ...
      <form @submit.prevent="saveArticle" class="space-y-4">
        ...
        >
          <div class="flex space-x-2">
            ...
            <button
              type="button"
              class="text-red-500 h-5 w-5 text-[12px] rounded-sm border-b border-[--primary]"
          ...
          >
            <div class="flex space-x-2">
              ...
              <button
                type="button"
                class="p-0 text-red-500 h-5 w-5 text-[12px]"
    ...
    <section
      ...
      class="w-full flex flex-col gap-0 bg-[#EAEAEA38]"
    >
      <div
        class="flex flex-col gap-5 px-6 py-8 sm:pt-[64px] sm:pb-[80px] items-center justify-center bg-gradient-to-b from-[#FFFFFF] to-[#fef8f3] border-b border-[#0A0A0A12]"
      >
        <h1
          class="text-[28px] leading-[33px] sm:text-[48px] font-bold w-full max-w-[80%] text-center text-[#1D1D1D]"
        >
          {{ article.title }}
        </h1>
        <p
          v-show="article.description"
      ...
      <div class="px-4 lg:px-6 py-20 flex flex-col items-center bg-white">
        <div class="flex flex-col w-full lg:max-w-[70%] items-center gap-6">
          <img
            v-if="article.image && article.image.startsWith('http')"
            :src="article.image"
            loading="lazy"
            class="w-full max-h-[320px] mx-auto object-cover rounded-2xl"
          />
          ...
          <div
            class="grid grid-cols-3 whitespace-nowrap rounded-xl px-2 py-4 sm:p-4 bg-[#F7F7F7] w-full text-[12px] leading-[20px] sm:text-[18px] font-[400]"
          >
    ...
    <section
      v-show="!article.title"
      class="w-full bg-[#F7F7F7] flex justify-center items-center"
    >
