...
const stepsBlock = ref([
  {
    ...
    image:
      "https://ozonmpportal.hb.vkcs.cloud/harmex/landing1/img/mpSteps/step1.png",
  },
  {
    title: "Выбор услуги",
    description:
    ...
    image:
      "https://ozonmpportal.hb.vkcs.cloud/harmex/landing1/img/mpSteps/step2.png",
  },
  {
    ...
    image:
      "https://ozonmpportal.hb.vkcs.cloud/harmex/landing1/img/mpSteps/step3.png",
  },
  {
    title: "Отчётность",
    description:
    ...
    image:
      "https://ozonmpportal.hb.vkcs.cloud/harmex/landing1/img/mpSteps/step4.png",
  },
]);
...
onMounted(() => {
  const mplace = route.params.slug;
  if(store.info[mplace]) {
    persistedStore.current = mplace
  }
  // persistedStore.current
  //   ? persistedStore.current
  //   : (persistedStore.current = "wildberries");
  store.info[persistedStore.current].title
    ? (mp.value = store.info[persistedStore.current])
    : navigateTo("/");
  getBlockHeight();
...
onMounted(() => {
  ...
  setInterval(() => {
    const currentIndex = content.findIndex(
      (item) => item.value === currentContent.value
    );
...
<template>
  <div class="flex flex-col gap-0 bg-[#EAEAEA38]">
    ...
    >
      <h1
        class="text-[28px] leading-[33px] sm:text-[48px] sm:leading-[56px] font-[700] max-w-[632px] text-center text-[#1D1D1D]"
      >
        Репутация и продажи под вашим контролем на {{ mp.title }}!
      </h1>
      <p
        class="text-[18px] leading-[28px] font-[400] text-[#0A0A0AB2] max-w-[480px] text-center"
      >
        Увеличивайте заказы и положительные отзывы 24/7 с Harmex — ваш надежный
        помощник в борьбе за высокий рейтинг и доверие клиентов!
      </p>
      <div class="flex gap-2">
        <button
          type="button"
          aria-label="Попробовать"
          @click="navigateTo('https://app.harmex.ru/auth', { external: true })"
          class="btn-primary lg:h-[60px] lg:!px-[32px] !max-h-[40px] flex items-center"
        >
          Попробовать
        ...
        <a href="#contact" alt="Перейти в блок обратный звонок">
          <button
            ...
            class="btn-primary flex gap-3 !bg-white !font-[400] !text-black !max-h-[40px]"
          >
    ...
    <section
      class="bg-white px-6 lg:px-[120px] py-12 lg:py-14 flex flex-col gap-14 pb-[136px] lg:pb-14"
    >
      <div class="flex flex-col justify-center items-center gap-5">
        ...
        <p
          class="text-[18px] leading-[28px] font-[400] text-[#0A0A0AB2] max-w-[480px] text-center"
        >
          Скрытые ошибки, которые мешают вам выйти в топ и увеличить продажи на
          {{ mp.title }}
        </p>
      </div>
      <div class="flex justify-between gap-8 lg:w-[50%] max-w-[600px] mx-auto">
        <div class="flex flex-col gap-6 items-center" v-for="item in content">
          <button
            type="button"
            @click="[(currentContent = item.value)]"
            class="flex justify-center items-center btn-primary w-16 h-16"
            :class="{
              '!bg-[#F7F7F7] !text-black': item.value !== currentContent,
            }"
          >
            <Icon :name="item.icon" class="w-7 h-7" />
          </button>
          <p class="md:text-[20px] text-[12px] font-[600] leading-[30px]">
            {{ item.title }}
          </p>
        </div>
      </div>
      <div
        id="slideshow"
        class="relative overflow-hidden min-h-[400px] w-full -mb-32 lg:-mb-0 sm:my-0 z-0"
        :style="dynamicHeight"
      >
        <transition-group name="fade" tag="div">
          ...
          >
            <div
              class="flex w-full justify-between lg:flex-row flex-col p-4 md:p-12 gap-4 md:gap-12 bg-[#F7F7F7] rounded-2xl overflow-hidden"
              ref="contentBlock"
              :style="blockHeight !== 0 ? 'height: ' + blockHeight + 'px' : ''"
            >
              <div class="flex flex-col gap-3 md:gap-8 flex-1">
                <p
                  class="text-[18px] leading-[24px] md:font-[600] md:text-[24px] md:leading-[29px]"
                >
                  Как эту задачу решил Harmex:
                </p>
                <div
                  class="flex gap-3 items-center"
                  v-for="item in content.find(
                    (item) => item.value === currentContent
                  ).points"
                >
                  <div
                  ...
                  <p
                    class="text-[#5A556B] text-[12px] lg:text-[16px] leading-[24px] font-[500]"
                  >
              ...
              <div
                class="relative min-h-[300px] lg:w-[45%] lg:max-w-[566px] max-h-[400px] lg:max-h-full lg:h-full flex lg:my-auto overflow-hidden rounded-xl"
              >
                <img
    ...
    <section
      class="bg-white py-14 px-6 lg:px-[120px] lg:pt-14 lg:pb-[112px] flex flex-col gap-8 lg:gap-14 justify-center items-center w-full"
    >
      <h1 class="block-title text-center">Почему выбирают HARMEX?</h1>
      <img
        src="https://ozonmpportal.hb.vkcs.cloud/harmex/landing1/img/harmex.svg"
        alt="image"
        class="w-full max-w-[1120px] max-h-[700px] flex mx-auto"
        loading="lazy"
      />
      ...
      <a href="#contact" alt="Перейти в блок обратный звонок">
        <button
          type="button"
          aria-label="Обратный звонок"
          class="btn-primary flex gap-3 !rounded-3xl !bg-[#04201A] !font-[400]"
        >
          <Icon name="lucide:phone" class="w-5 h-5" />
          <span>Обратный звонок</span>
        </button>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <div class="flex flex-col gap-1">
            <li
              class="flex w-full items-center after:content-[''] after:w-full after:h-1 after:border-b after:border-1 after:inline-block"
              :class="{
                'after:bg-[linear-gradient(to_right,_#6366F1_50%,_white_50%)]':
                  index === 0,
                'after:bg-white': index !== 0,
              }"
            >
              <span
                class="flex items-center justify-center w-8 h-8 rounded-full shrink-0"
                :class="{
                  'bg-[#6366F1] text-white': index === 0,
                  'bg-white border': index !== 0,
                }"
              >
                {{ index + 1 }}
              </span>
            </li>
            <p class="font-[600] text-[20px] leading-[28px] mr-6">
              {{ item.title }}
            </p>
            <p class="text-[#0A0A0AB2] text-[18px] leading-[28px] mr-6">
              {{ item.description }}
            </p>
          </div>
        </div>
    ...
    <section
      class="bg-white flex flex-col py-0 px-0 lg:px-16 items-center justify-center"
    >
      <div
        id="slideshow"
        class="relative overflow-hidden h-[400px] lg:min-h-[800px] w-full -mt-5 -mb-10 sm:my-0 z-0"
      >
        <transition-group name="fade" tag="div">
          <div
      ...
      <a
        href="#contact"
        class="my-10 -mt-10 sm:mt-0 lg:-mt-32 xl:mt-20 z-10"
        alt="Перейти в блок обратный звонок"
      >
        <button
          type="button"
          aria-label="Обратный звонок"
      ...
      <div class="flex lg:flex-row flex-col gap-5">
        ...
        >
          <div class="flex justify-between">
            <p class="text-[16px] leading-[28px] text-[#667085]">
              <span class="emoji-container">{{ emojis[key] }}</span>
              {{ item.title }}
            </p>
            <span
          ...
          <p class="text-[40px] sm:text-[50px] leading-[60px] font-[600]">
            от {{ item.value }}
          </p>
          <p class="text-[#667085] text-[16px] leading-[24px]">
            {{ item.description }}
          </p>
          <button
            type="button"
            aria-label="Выбрать тариф"
            class="mt-4 btn-primary h-[48px] text-[16px] leading-[24px]"
            @click="
    ...
    <section
      class="w-full rounded-[12px] flex items-center lg:px-[64px] py-8 px-6 lg:py-20 bg-white flex-col"
    >
      <h1
      ...
      <p class="text-[#4B5563] text-[16px] font-[400] leading-[25px] mb-8">
        Сравните карточки до работы с нами (низкий рейтинг) и после (4.9).
        Примеры наглядно показывают, как изменяется эффективность.
      </p>
      <Slider />
    </section>
    ...
    >
      ...
      >
        <div class="flex lg:flex-row flex-col gap-1.5">
          ...
          />
          <button type="button" aria-label="Отправить" class="btn-black">
            Отправить
          </button>
        </div>
